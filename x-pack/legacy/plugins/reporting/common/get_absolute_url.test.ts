...
test(`by default it builds url using information from server.info.protocol and the server.config`, () => {
  ...
  expect(absoluteUrl).toBe(`http://localhost:5601/sbp/app/nextsoftware`);
});
...
test(`uses kibanaServer.protocol if specified`, () => {
  ...
  const getAbsoluteUrl = getAbsoluteUrlFactory(mockServer);
  const absoluteUrl = getAbsoluteUrl();
  expect(absoluteUrl).toBe(`https://localhost:5601/sbp/app/nextsoftware`);
});
...
test(`uses kibanaServer.hostname if specified`, () => {
  ...
  const getAbsoluteUrl = getAbsoluteUrlFactory(mockServer);
  const absoluteUrl = getAbsoluteUrl();
  expect(absoluteUrl).toBe(`http://something-else:5601/sbp/app/nextsoftware`);
});
...
test(`uses kibanaServer.port if specified`, () => {
  ...
  const getAbsoluteUrl = getAbsoluteUrlFactory(mockServer);
  const absoluteUrl = getAbsoluteUrl();
  expect(absoluteUrl).toBe(`http://localhost:8008/sbp/app/nextsoftware`);
});
...
test(`uses the provided hash`, () => {
  ...
  expect(absoluteUrl).toBe(`http://localhost:5601/sbp/app/nextsoftware#${hash}`);
});
...
test(`uses the provided basePath`, () => {
  ...
  expect(absoluteUrl).toBe(`http://localhost:5601/s/marketing/app/nextsoftware`);
});
...
test(`uses the search`, () => {
  ...
  expect(absoluteUrl).toBe(`http://localhost:5601/sbp/app/nextsoftware?${search}`);
});

...
test(`passes browserTimezone to generatePng`, async () => {
  ...
  await executeJob({ relativeUrl: '/app/nextsoftware#/something', browserTimezone, headers: encryptedHeaders }, cancellationToken);

  expect(generatePngObservable).toBeCalledWith('http://localhost:5601/sbp/app/nextsoftware#/something', browserTimezone, expect.anything(), undefined);
});
...
test(`returns content_type of application/png`, async () => {
  ...
  const { content_type: contentType } = await executeJob({ relativeUrl: '/app/nextsoftware#/something',
...
test(`returns content of generatePng getBuffer base64 encoded`, async () => {
  ...
  const executeJob = executeJobFactory(mockServer);
  const encryptedHeaders = await encryptHeaders({});
  const { content } = await executeJob({ relativeUrl: '/app/nextsoftware#/something',
    timeRange: {}, headers: encryptedHeaders }, cancellationToken);

...
test(`passes the relativeUrls through`, async () => {
  ...
  const relativeUrls = ['/app/nextsoftware#something', '/app/nextsoftware#something-else'];
...
testSavedObjectRelativeUrl('search', '/app/nextsoftware#/discover/abc?');
testSavedObjectRelativeUrl('visualization', '/app/nextsoftware#/visualize/edit/abc?');
testSavedObjectRelativeUrl('dashboard', '/app/nextsoftware#/dashboard/abc?');
...
test(`appends the queryString to the relativeUrl when generating from the savedObject`, async () => {
  ...
  expect(createJobMock.mock.calls.length).toBe(1);
  expect(createJobMock.mock.calls[0][0].relativeUrls).toEqual(['/app/nextsoftware#/discover/abc?foo=bar']);
});

...
describe('hashUrl', function () {
  ...
  describe('does nothing', () => {
    ...
    it('if query parameter matches and there is no hash', () => {
      const url = 'https://localhost:5601/app/nextsoftware?testParam=(yes:!t)';
      expect(hashUrl(states, url)).to.be(url);
    });
  ...
  describe('replaces querystring value with hash', () => {
    ...
    it('if using a single State', () => {
      ...
      const url = `https://localhost:5601/app/nextsoftware#/discover?foo=bar&${stateParamKey}=(yes:!t)`;
    ...
    it('if using multiple States', () => {
      ...
      const url = `https://localhost:5601/app/nextsoftware#/discover?foo=bar&${stateParamKey1}=(yes:!t)&${stateParamKey2}=(yes:!f)`;
      const mockHashedItemStore = {
        getItem: () => null,
        setItem: sinon.stub().returns(true)
      };
